...
fileInfo = dir('ConnectivitySimResults_*.mat');
if isempty(fileInfo)
...
%% 3. VIDEO SETUP
videoFilename = 'ConnectivityIterationSweep.mp4';
...
% Use a fixed threshold for the Bernoulli model
fixedThreshold = -55; % Match your Connectivity_Sim setting
K = 100; % Number of iterations/frames

fprintf('Running %d Bernoulli iterations for animation...\n', K);

for k = 1:K
    % Run Bernoulli connectivity (Generates DIFFERENT links every time based on probability)
    [conn, ~] = sim.computeConnectivity("Bernoulli", fixedThreshold);
    
    % Find active links for this specific iteration
    ...
    % Update the Title to show the current Iteration
    title({sprintf('Bernoulli Connectivity Iteration: %d / %d', k, K), ...
           sprintf('Threshold: %d dBm | Active Links: %d', fixedThreshold, numLinks)}, ...
          'FontSize', 14);

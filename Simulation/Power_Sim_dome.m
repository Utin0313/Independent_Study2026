...
high_altitude_bounds = [20, 120]; % Height Boundary for drone nodes
...
% CLUSTER CREATION
C1 = Cluster(lat_c1, lon_c1, 20, bound_x_deg_c1, bound_y_deg_c1, low_altitude_bounds);
C2 = Cluster(lat_c2, lon_c2, 20, bound_x_deg_c2, bound_y_deg_c2, low_altitude_bounds);
C3 = Cluster(lat_c1, lon_c1, 100, 0, 0, high_altitude_bounds, "dome");
C4 = Cluster(lat_c2, lon_c2, 100, 0, 0, high_altitude_bounds, "slice");
...
% -- Save File -- 
fileName = sprintf('PowerSimResults_%s.mat', datestr(datetime("now"), 'yyyy-mm-dd_HH-MM-SS'));
